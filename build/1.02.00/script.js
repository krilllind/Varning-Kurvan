...
function startGame() {
	for (var i = 0; i < map.length; i++) {
		ctx.beginPath();
		ctx.rect(map[i].x, map[i].y, map[i].w, map[i].h);
...
function initGame() {
	// Load in settings from settings panel
	$.each($(".option"), function(key, val) {
		switch(val.id) {
			case "useGaps":
				if(val.checked)
					gameHandler.useGaps = true;
				else
					gameHandler.useGaps = false;
			break;

			case "map":
				if(val.checked)
					gameHandler.map = val.value;
			break;
	...
	$.ajax({
		url: "./bin/maps/"+gameHandler.map+".json",
		dataType: "json",
		async: false,
